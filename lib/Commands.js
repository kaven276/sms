...
Msg.prototype.makePDU = function(){
  var me = this
    , PDULen = 20 + me.getPDULength()
      ...
      case 'text' :
        (function(len){
          ...
          if (len) {
            ...
            bytesWriten = PDU.write(val, sPtr);
...
Msg.parse = function(data){
  var PDULen = data.readUInt32BE(0)
    , MsgType = CmdMap[data.readUInt32BE(4)]
    , msg = new MsgType()
...
(function export_commands(){
  ...
  cmds.split(',').forEach(function(cmd, i){
    ...
    if (!mCmd.Resp) {
      mCmd.Resp = new Function(cmd || 'Resp', '');
      mCmd.Resp.PDUAttrSeq = [A.Result];
    }
    exports[cmd + 'Resp'] = mCmd.Resp;
    proto = mCmd.Resp.prototype;
    util.inherits(mCmd.Resp, Msg);
    for (var n in proto) {
      mCmd.Resp.prototype[n] = proto[n];
    }
    Msg.setFixedPartLength(mCmd.Resp);
    mCmd.Resp.code = 0x80000000 + i;
  });

  CmdMap[0x1000] = CmdMap.pop();
  CmdMap[0x1000].code = 0x1000;
  CmdMap[0x1000].Resp.code = 0x80001000;
  CmdMap[0x80001000] = CmdMap[0x1000].Resp;
})();
